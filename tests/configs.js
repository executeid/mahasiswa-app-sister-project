// tests/configs.js
const { v4: uuidv4 } = require('uuid'); // Pastikan uuid terinstal

const KUBERNETES_INGRESS_IP = "k8s-worker-01"; // IP Worker Node Anda (misal: k8s-worker-01)
const KUBERNETES_INGRESS_NODEPORT = "31472"; // NodePort yang diekspos Ingress Nginx Anda

const BASE_URL = `http://${KUBERNETES_INGRESS_IP}:${KUBERNETES_INGRESS_NODEPORT}`;

const API_ENDPOINTS = {
    mahasiswa: `${BASE_URL}/mahasiswa`,
    academic: `${BASE_URL}/api/academic`,
    health: `${BASE_URL}/health`
};

// --- BAGIAN KAFKA BROKERS YANG SANGAT PENTING ---
// Pastikan ini adalah IP Pod Kafka internal cluster Anda dan tidak ada typo
// Gunakan 'kubectl get pods -l app=kafka -o wide' untuk mendapatkan IP Pod Kafka yang Running
const KAFKA_BROKERS_LIST_STRING = '10.244.1.105:9092'; // Pastikan IP ini benar dan sesuai dengan pod Kafka Anda
const KAFKA_BROKERS = ['localhost:19092']; // Tambahkan array untuk brokers

console.log('DEBUG Kafka brokers:', KAFKA_BROKERS); // Debug log untuk melihat nilai KAFKA_BROKERS

const KAFKA_TOPIC_MAHASISWA_EVENTS = 'mahasiswa_events';
const KAFKA_TOPIC_ACADEMIC_EVENTS = 'academic_events';

// --- KONFIGURASI DB ---
// Pastikan IP Pod PostgreSQL Log DB Anda benar
const DB_CONFIG_LOG = {
    user: 'log_user',
    host: '10.244.1.99', // Ganti dengan IP Pod PostgreSQL Log DB yang Running
    database: 'log_db',
    password: 'akmal', // Ganti dengan password log_user Anda
    port: 5432,
};

// Pastikan IP Pod PostgreSQL Mahasiswa DB Anda benar
const DB_CONFIG_MAHASISWA = {
    user: 'mahasiswa_user',
    host: '10.244.2.92', // Ganti dengan IP Pod PostgreSQL Mahasiswa DB yang Running
    database: 'mahasiswa_db',
    password: 'akmal', // Ganti dengan password mahasiswa_user Anda
    port: 5432,
};

// Pastikan IP Pod PostgreSQL Academic DB Anda benar
const DB_CONFIG_ACADEMIC = {
    user: 'academic_user',
    host: '10.244.1.82', // Ganti dengan IP Pod PostgreSQL Academic DB yang Running
    database: 'academic_db',
    password: 'akmal', // Ganti dengan password academic_user Anda
    port: 5432,
};
// --- AKHIR KONFIGURASI DB ---

// Kredensial Dosen untuk Testing
const LECTURER_CREDENTIALS = {
    nidn: "TEST_D001",
    name: "Dr. Test Dosen",
    email: "test.dosen@example.com",
    password: "testpassword"
};

module.exports = {
    API_ENDPOINTS,
    KAFKA_BROKERS, // Ekspor array, gunakan ini di testRunner/utils
    KAFKA_BROKERS_LIST_STRING, // (Opsional, jika masih ingin diekspor)
    KAFKA_TOPIC_MAHASISWA_EVENTS,
    KAFKA_TOPIC_ACADEMIC_EVENTS,
    DB_CONFIG_LOG,
    DB_CONFIG_MAHASISWA,
    DB_CONFIG_ACADEMIC,
    LECTURER_CREDENTIALS,
    uuidv4
};
